...
const ContentRenderer = ({ content }) => {
  if (content.type === "doc" || content.type === "paragraph") {
    if (!content.content) return <Text></Text>;
    return content.content.map((item) => <ContentRenderer content={item} />);
  }
...
const MyDocument = ({ document }) => {
  ...
  return (
    <Document pdfVersion="1.3">
      <Page style={styles.body} size="FOLIO" orientation="landscape">
        ...
        {document && <ContentRenderer content={document} />}
